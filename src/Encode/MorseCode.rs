    ...
    pub fn display_all_morse_code(&self) {
        ...
        for (key, value) in &self.morse_code {
            writeln!(
                ...
                "Key: {}  {}",
                key,
                value.iter().map(|pulse| pulse.value()).collect::<String>()
            )
    ...
    pub fn morse_to_text(&self, morse_code: &str) -> Option<String> {
        ...
        for (i, word) in words.iter().enumerate() {
            ...
            for (j, letter) in letters.iter().enumerate() {
                ...
                let decoded_letter = self.morse_code.iter().find_map(|(key, value)| {
...
pub fn morse_code_encode(plain_text: &str) -> String {
    MorseCode::new().text_to_morse(plain_text)
}

pub fn morse_code_decode(cipher: &str) -> Option<String> {
    MorseCode::new().morse_to_text(cipher)
}
